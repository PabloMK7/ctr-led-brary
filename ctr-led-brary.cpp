...
RGBLedPattern LED::GeneratePattern(Color color, LED_PatType type, float delay_time, float loop_delay, u8 smooth, float r_shift, float g_shift, float b_shift) {
	RGBLedPattern pat = {0};
	...
	switch(type) {
		case (LED_PatType::WAVE_DESC):			
			for(int i = 0; i < 32; i++) {
				x = ((float)i/31) * 2*M_PI;
				pat.r[i] = ((cos(x + r_shift) / 2) + 0.5) * color.r;
				pat.g[i] = ((cos(x + g_shift) / 2) + 0.5) * color.g;
				pat.b[i] = ((cos(x + b_shift) / 2) + 0.5) * color.b;
			}
			break;
		case (LED_PatType::WAVE_ASC):
			for(int i = 0; i < 32; i++) {
				x = ((float)i/31) * 2*M_PI;
				pat.r[i] = ((sin(x + r_shift + (3*M_PI)/2) / 2) + 0.5) * color.r;
				pat.g[i] = ((sin(x + g_shift + (3*M_PI)/2) / 2) + 0.5) * color.g;
				pat.b[i] = ((sin(x + b_shift + (3*M_PI)/2) / 2) + 0.5) * color.b;
			}
			break;
		case (LED_PatType::ASCENDENT):
			for(int i = 0; i < 32; i++) {
				xr = modf((float)i/32 + r_shift/2*M_PI, &dummy);
				xg = modf((float)i/32 + g_shift/2*M_PI, &dummy);
				xb = modf((float)i/32 + b_shift/2*M_PI, &dummy);
		...
		case (LED_PatType::DESCENDENT):
			for(int i = 0; i < 32; i++) {
				xr = 1 - modf((float)i/32 + r_shift/2*M_PI, &dummy);
				xg = 1 - modf((float)i/32 + g_shift/2*M_PI, &dummy);
				xb = 1 - modf((float)i/32 + b_shift/2*M_PI, &dummy);
				pat.r[i] = (u8)(xr * color.r);
				pat.g[i] = (u8)(xg * color.g);
				pat.b[i] = (u8)(xb * color.b);
			}
